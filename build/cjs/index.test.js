...
describe('deeplyCopy', () => {
    test('primitive type을 복사한다. ', () => {
        expect((0, index_1.deeplyCopy)(12)).toBe(12);
        expect((0, index_1.deeplyCopy)('dummy')).toBe('dummy');
        expect((0, index_1.deeplyCopy)(false)).toBe(false);
        expect((0, index_1.deeplyCopy)(undefined)).toBe(undefined);
        expect((0, index_1.deeplyCopy)(null)).toBeNull();
        expect((0, index_1.deeplyCopy)(Symbol('foo'))).toBe('foo');
    });
    test('단일 레벨 객체를 복사한다.', () => {
        const obj = { a: 'foo', b: 'bar' };
        const copied = (0, index_1.deeplyCopy)(obj);
        expect(copied).toEqual(obj);
        expect(copied).not.toBe(obj);
    });
    test('단일 레벨 배열을 복사한다.', () => {
        const arr = [1, 2, 3];
        const copied = (0, index_1.deeplyCopy)(arr);
        expect(copied).toEqual(arr);
        expect(copied).not.toBe(arr);
    });
    test('중첩 객체를 내부까지 복사한다.', () => {
        const nestedObj = { a: { b: 'foo' }, c: { d: { e: 'bar' }, f: 'baz' } };
        const copied = (0, index_1.deeplyCopy)(nestedObj);
        expect(copied).toEqual(nestedObj);
        expect(copied).not.toBe(nestedObj);
    });
    test('중첩 배열을 내부까지 복사한다.', () => {
        const nestedArr = [[0], [[1], [2]], [[[3]]]];
        const copied = (0, index_1.deeplyCopy)(nestedArr);
    ...
    test('클래스 인스턴스를 복사한다.', () => {
        ...
        node1.next = node2;
        node2.next = node1;
        const copiedNode1 = (0, index_1.deeplyCopy)(node1);
        ...
        expect(copiedNode1.next.next).toBe(copiedNode1);
    });
});
